    ...
    return (
        <div className="bg-neutral-50 flex flex-col divide-y rounded-md" data-order-id={order?.id}>
            ...
            <div className="p-4 flex flex-col gap-2">
                ...
                <div>
                    {getLength(order.orderitems) > 1 ? <span className="text-base text-custom_gray font-normal">
                        {`${trimToLength(getFirst(order?.orderitems)?.inventory?.product?.name, 10)} + ${getLength(order.orderitems) -1} items`}
                    </span>
                        : <span className="text-base text-custom_gray font-normal">{trimToLength(getFirst(order.orderitems)?.inventory?.product?.name, 20)}</span>
                    }
                </div>
