...
export const SmallProductCard = ({ product }: Props) => {
    ...
    return (
        <div className="flex flex-col bg-neutral-50 rounded-md">
            <div className="grid grid-cols-3 items-start px-2 py-2 lt-sm:px-0 gap-4" >
                ...
                <div className="flex flex-col gap-4 justify-start col-span-2">
                    <div className="flex flex-col gap-2 leading-tight">
                        <Link href={`/product/${product.id}/${getSafeUrl(product?.name)}`}>
                            <div className="font-medium truncate text-ellipsis text-base lt-sm:text-base">{trimToLength(getCombinedName(product, variant?.id), 40)}</div>
                        </Link>
                        ...
                        {getLength(product?.inventories) > 0 ?
                            <div className="price">
                                <span className="text-custom_black font-bold text-xl lt-sm:text-base">{humanizeCurrency(getFirst(product?.inventories)?.price || product?.mrp)}</span>
