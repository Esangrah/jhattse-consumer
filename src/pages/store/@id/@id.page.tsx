    ...
    return {
        pageContext: {
            ...
            documentProps: {
                ...
                canonicalURL: `https://jhattse.com/store/${initialStore.id}/${getSafeUrl(initialStore?.name)}`,
            }
        }
    }
}
...
export const Page: React.FC<Props> = ({ initialStore }: Props) => {
    ...
    useEffect(() => {
        ...
        if (pageContext.urlParsed?.search?.source && pageContext.urlParsed?.search?.table_id) {
            localStorage.setItem("tableInfo", JSON.stringify({ source: pageContext.urlParsed?.search?.source, table: pageContext.urlParsed?.search?.table, table_id: pageContext.urlParsed?.search?.table_id, is_ac: pageContext.urlParsed?.search?.is_ac }));
        }
                                                ...
                                                <div className="flex flex-row justify-between items-center">
                                                    ...
                                                    {index === 0 &&
                                                        <div className="lt-sm:hidden flex justify-center w-2/5">
                                                            ...
                                                            >
                                                                <Input size="large" placeholder="Search in Store" prefix={<MdSearch className='font-bold' size="1.5em" />} />
                                                            </AutoComplete>
                                                        </div>
                                                    }
                                                </div>
                    ...
                    <div className='hidden lt-sm:block fixed bottom-10 left-1/3 z-20'>
                        <Menu
                            ...
                            <MenuButton as={Button} className="bg-neutral-50 font-bold font-sm tracking-wide text-custom_gray border-neutral-400 rounded-sm">
                                {!isCategoryOpen ? "CATEGORIES" : <div className='flex flex-row gap-1 items-center'> <MdOutlineClose className="inline" /> CLOSE</div>}
                            </MenuButton>
                            <Center>
